---
layout: post
title: Great Links
date: 2020-02-08 22:12 -0500
author: Chauncy
category: 
tags: 
image: 
published: true
---


## Web
- [MVC vs. MVVM: How a Website Communicates With Its Data Models](https://hackernoon.com/mvc-vs-mvvm-how-a-website-communicates-with-its-data-models-18553877bf7d)
- [PUT vs. POST in REST](https://stackoverflow.com/questions/630453/put-vs-post-in-rest)
- [Distributed Monolith vs. Microservices Architecture: 4 Ways New Relic Can Tell You Which Is Which](https://blog.newrelic.com/engineering/distributed-monolith-vs-microservices/)
- [Sharding & IDs at Instagram](https://instagram-engineering.com/sharding-ids-at-instagram-1cf5a71e5a5c)
- [Design Patterns web based applications](https://stackoverflow.com/questions/3541077/design-patterns-web-based-applications/3542297#3542297)


## Database
- [How does database indexing work?](https://stackoverflow.com/questions/1108/how-does-database-indexing-work)
- [Database sharding vs partitioning](https://stackoverflow.com/questions/20771435/database-sharding-vs-partitioning)
- [What is the “N+1 selects problem” in ORM (Object-Relational Mapping)?](https://stackoverflow.com/questions/97197/what-is-the-n1-selects-problem-in-orm-object-relational-mapping)


## Spring / J2EE
- [Spring Transaction Management Example with JDBC Example](https://examples.javacodegeeks.com/enterprise-java/spring/jdbc/spring-transaction-management-example-with-jdbc-example/)
- [What is the use of DataSourceTransactionManager And JndiObjectFactoryBean And JdbcTemplate?](https://stackoverflow.com/questions/40086182/what-is-the-use-of-datasourcetransactionmanager-and-jndiobjectfactorybean-and-jd/40090174#40090174)
- [3 Difference between Web Server vs Application vs Servlet Containers in Java JEE](https://www.java67.com/2016/06/3-difference-between-web-server-vs-application-server-vs-servlet-container.html)
- [How do servlets work? Instantiation, sessions, shared variables and multithreading](https://stackoverflow.com/questions/3106452/how-do-servlets-work-instantiation-sessions-shared-variables-and-multithreadi)
- [How you should unit test DAO layer](https://howtodoinjava.com/best-practices/how-you-should-unit-test-dao-layer/)
- [Everything You Should Know About Spring IOC](https://blog.csdn.net/javazejian/article/details/54561302)


## Python
- [Dicts are now ordered, get used to it](https://softwaremaniacs.org/blog/2020/02/05/dicts-ordered/en/)


## NLP
- [Tf-idf weighting](https://nlp.stanford.edu/IR-book/html/htmledition/tf-idf-weighting-1.html)
